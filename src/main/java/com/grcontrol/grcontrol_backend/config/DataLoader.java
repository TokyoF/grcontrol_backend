...
public class DataLoader implements CommandLineRunner {
    ...
    private void createSampleWorkerAssignment() {
        ...
        if (juan != null) {
            createWeeklyAssignment(juan, island1, morningShift, monday, DayOfWeek.SUNDAY);
            log.info("  ✓ Juan: Isla 1, Turno Mañana, L-S (descansa D)");
        }

        // Olivia - Isla 2 - Turno Mañana - L a S (descansa D)
        if (olivia != null) {
            createWeeklyAssignment(olivia, island2, morningShift, monday, DayOfWeek.SUNDAY);
            log.info("  ✓ Olivia: Isla 2, Turno Mañana, L-S (descansa D)");
        }

        // Luis - Isla 3 (GLP) - Turno Mañana - L a S (descansa D)
        if (luis != null) {
            createWeeklyAssignment(luis, island3, morningShift, monday, DayOfWeek.SUNDAY);
        ...
        if (ana != null) {
            createWeeklyAssignment(ana, island1, afternoonShift, monday, DayOfWeek.MONDAY);
            log.info("  ✓ Ana: Isla 1, Turno Tarde, M-D (descansa L)");
        }

        // Pedro - Isla 2 - Turno Tarde - M a D (descansa L)
        if (pedro != null) {
            createWeeklyAssignment(pedro, island2, afternoonShift, monday, DayOfWeek.MONDAY);
            log.info("  ✓ Pedro: Isla 2, Turno Tarde, M-D (descansa L)");
        }

        // Sofia - Isla 3 (GLP) - Turno Tarde - M a D (descansa L)
        if (sofia != null) {
            createWeeklyAssignment(sofia, island3, afternoonShift, monday, DayOfWeek.MONDAY);
        ...
        if (miguel != null) {
            createWeeklyAssignmentWithTwoRestDays(miguel, island1, nightShift, monday, 
        ...
        if (carmen != null) {
            createWeeklyAssignmentWithTwoRestDays(carmen, island2, nightShift, monday, 
                DayOfWeek.MONDAY, DayOfWeek.TUESDAY);
        ...
        if (roberto != null) {
            createWeeklyAssignmentWithTwoRestDays(roberto, island3, nightShift, monday, 
                DayOfWeek.MONDAY, DayOfWeek.TUESDAY);
        ...
        if (laura != null) {
            ...
            createSingleAssignment(laura, island1, morningShift, monday, DayOfWeek.SUNDAY, false);
            
            // Laura cubre el LUNES en Isla 1, Turno Tarde (día libre de Ana)
            createSingleAssignment(laura, island1, afternoonShift, monday, DayOfWeek.MONDAY, false);
            
            // Laura cubre el LUNES en Isla 1, Turno Noche (día libre de Miguel)
            createSingleAssignment(laura, island1, nightShift, monday, DayOfWeek.MONDAY, false);
            
            // Laura descansa Martes a Sábado
            for (DayOfWeek day : new DayOfWeek[]{DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY, 
                DayOfWeek.THURSDAY, DayOfWeek.FRIDAY, DayOfWeek.SATURDAY}) {
                createSingleAssignment(laura, null, null, monday, day, true);
            }
     ...
     */
    private void createWeeklyAssignment(User worker, Island island, ShiftSchedule shift, 
        ...
        for (DayOfWeek day : DayOfWeek.values()) {
            ...
            if (day == restDay) {
                assignment.setIsland(null);
                assignment.setShiftSchedule(null);
     ...
     */
    private void createWeeklyAssignmentWithTwoRestDays(User worker, Island island, ShiftSchedule shift, 
    ...
    private void createSingleAssignment(User worker, Island island, ShiftSchedule shift, 
                                       LocalDate monday, DayOfWeek day, boolean isRestDay) {
        WorkerAssignment assignment = new WorkerAssignment();
        assignment.setWorker(worker);
        assignment.setWeekStartDate(monday);
        assignment.setDayOfWeek(day);
    ...
    private void initializeBaseReadings() {
        ...
        // Obtener todas las mangueras con relaciones cargadas (JOIN FETCH para evitar LazyInitializationException)
        ...
        baseSession.setShiftTime("6-14");
        baseSession.setStartTime(LocalDateTime.now().minusDays(1));
        baseSession.setEndTime(LocalDateTime.now().minusDays(1).plusHours(8));
    ...
    private void createInitialReading(
        ...
        reading.setSide(nozzle.getSide() == Nozzle.PumpSide.LEFT ?
            PumpReading.PumpSide.LEFT : PumpReading.PumpSide.RIGHT);
